...
class RoundManager(QWidget):
    ...
    def create_round_table(self, round):
        ...
        if round.matches and len(round.matches) > 0:
            ...
            for match in round.matches:
                ...
                matches_item.setFlags(
                    matches_item.flags() & ~Qt.ItemIsEditable
                    )
        ...
        self.save_btn.clicked.connect(
            lambda: self.next_round(
                combo_boxes_data,
                round,
                round_name.input.text(),
                start_date.input.dateTime(),
                end_date.input.dateTime(),
            )
        )

        self.next_auto = QPushButton("Save and simulate next round")
        self.next_auto.clicked.connect(
            lambda: self.simulate_next(
                combo_boxes_data,
                round,
                round_name.input.text(),
                start_date.input.dateTime(),
        ...
        self.all_auto.clicked.connect(
            lambda: self.simulate_all(
                combo_boxes_data,
                round,
                round_name.input.text(),
                start_date.input.dateTime(),
        ...
        end_date = InputField(
            self.layout, "Round end date", "Round end date", FormType.DateTime
        )
    ...
    def next_round(
        ...
        if (
            int(self.tournament.current_round)
    ...
    def simulate_next(
        ...
        if (
            ...
            self.nav.switch_to_tournament_step_by_step_simulator(
                self.tournament
                )
        else:
            self.message.setText("Tournament is over")
