...
class TournamentManagerView(QWidget):
    def __init__(self, nav, tournament, index=0):
        ...
        self.manually_add_players_btn.clicked.connect(
        ...
        self.auto_add_players_btn.clicked.connect(
        ...
        self.auto_add_players_button_validation.clicked.connect(
            self.auto_select_players
        )
        ...
        self.simulate_next_round.clicked.connect(
            self.start_step_by_step_simulation
            )
    ...
    def populate_table(self):
        ...
        start_date = QTableWidgetItem(
            self.tournament.start_date.toString(Qt.ISODate)
            )
        ...
        end_date = QTableWidgetItem(
            self.tournament.end_date.toString(Qt.ISODate)
            )
        ...
        registered_players = QTableWidgetItem(
            str(len(self.tournament._registered_players))
        )
        registered_players.setFlags(
            registered_players.flags() & ~Qt.ItemIsEnabled
            )
        ...
        self.table.setItem(
            row_position, self.registered_players_column, registered_players
        )
    ...
    def handle_item_changed(self, item):
        row = item.row()
        if self.cell_change_listener is True:
            try:
                id_item = self.table.item(row, self.id_main_table_column)
                tournament_id = id_item.text()
                edited_tournament = (
                    self.tournament_controller.tournament_repository
                    .find_one_by_id(
                        tournament_id
                    )
                )
                ...
                edited_tournament.start_date = QDateTime.fromString(
                    self.table.item(row, 2).text(), Qt.ISODate
                )
                edited_tournament.end_date = QDateTime.fromString(
                    self.table.item(row, 3).text(), Qt.ISODate
                )
                if (
                    int(self.table.item(row, 4).text())
                    < edited_tournament.current_round
                ):
                    min_allowed_value = QTableWidgetItem(
                        str(edited_tournament.current_round)
                    )
                ...
                edited_tournament.num_rounds = (
                    int(self.table.item(row, 4).text())
                    )
                edited_tournament.current_round = (
                    int(self.table.item(row, 5).text())
                    )
    ...
    def create_selected_players_table(self):
        ...
        self.selected_players_table.setHorizontalHeaderLabels(
            ["Player chess ID",
             "Player first name",
             "Player last name",
             "Select"]
        ...
        for player in self.all_players:
            ...
            player_chess_id.setFlags(
                player_chess_id.flags() & ~Qt.ItemIsEnabled
                )

            player_first_name = QTableWidgetItem(player.first_name)
            player_first_name.setFlags(
                player_first_name.flags() & ~Qt.ItemIsEnabled
                )

            player_last_name = QTableWidgetItem(player.last_name)
            player_last_name.setFlags(
                player_last_name.flags() & ~Qt.ItemIsEnabled
                )

            checkbox_widget = CenteredCheckBoxWidget()
            self.selected_players_table.insertRow(row_position)
            self.selected_players_table.setItem(row_position,
                                                0,
                                                player_chess_id
                                                )
            self.selected_players_table.setItem(row_position,
            ...
            self.selected_players_table.setCellWidget(
                row_position, self.id_player_table_column, checkbox_widget
            )

        self.layout.insertWidget(
            self.layout.indexOf(self.manually_add_players_btn) + 1,
            self.selected_players_table,
        )
        self.layout.insertWidget(
            self.layout.indexOf(self.selected_players_table) + 1,
            self.save_players_button,
        )
        # self.manually_add_players_btn.setVisible(False)

        for row in range(self.selected_players_table.rowCount()):
            checkbox_widget = self.selected_players_table.cellWidget(
                row, self.id_player_table_column
            )
            if isinstance(checkbox_widget, CenteredCheckBoxWidget):
                checkbox = checkbox_widget.layout().itemAt(0).widget()
                if self.tournament_controller.check_is_player_already_selected(
                    self.tournament.registered_players,
                    self.all_players,
                    self.selected_players_table.item(row, 0).text(),
                ):
    ...
    def create_main_table(self):
        ...
        self.layout.insertWidget(
            self.layout.indexOf(self.combo_box) + 1,
            self.table
            )

    def create_nbr_players_input(self):
        self.nbr_players_input = InputField(
            self.layout,
            "Number of players to implement",
            "Number of players",
            FormType.Numerical,
        )
        self.nbr_players_input.displace(
            self.layout.indexOf(self.auto_add_players_btn) + 1
        )
        self.nbr_players_input.input.textChanged.connect(
            self.toggle_auto_add_btn_validation
        )
        self.layout.insertWidget(
            self.layout.indexOf(self.nbr_players_input.input) + 1,
            self.auto_add_players_button_validation,
        )
        # self.auto_add_players_btn.setVisible(False)

    def toggle_auto_add_btn_validation(self, text):
        max_threshold = len(self.all_players)
        self.auto_add_players_button_validation.setVisible(
            True
        ) if text else self.auto_add_players_button_validation.setVisible(
            False
            )
        if text and int(text) > max_threshold:
            self.nbr_players_input.set_text(max_threshold)
    ...
    def toggle_add_player_button(self, tournament):
        if (
            int(tournament.current_round) > 1
            or int(tournament.num_rounds) == len(tournament.rounds)
        ):
            self.auto_add_players_btn.setVisible(False)
            self.auto_add_players_button_validation.setVisible(False)
            self.save_players_button.setVisible(False)
    ...
    def toggle_start_simulation_button(self, tournament):
        self.start_simulation_btn.setVisible(True) if (
            int(tournament.num_rounds) > len(tournament.rounds) and
            len(tournament.registered_players) > 0 and
            len(tournament.registered_players) % 2 == 0
        ) else self.start_simulation_btn.setVisible(False)

        self.manage_next_round.setVisible(True) if (
            int(tournament.num_rounds) > len(tournament.rounds) and
            len(tournament.registered_players) > 0 and
            len(tournament.registered_players) % 2 == 0
        ) else self.manage_next_round.setVisible(False)

        self.simulate_next_round.setVisible(True) if (
            int(tournament.num_rounds) > len(tournament.rounds) and
            len(tournament.registered_players) > 0 and
            len(tournament.registered_players) % 2 == 0
        ) else self.simulate_next_round.setVisible(False)

        self.view_round_button.setVisible(True) if (
            len(tournament.rounds) > 0 and
            len(tournament.registered_players) > 0 and
            len(tournament.registered_players) % 2 == 0
        ) else self.view_round_button.setVisible(False)
    ...
    def auto_select_players(self):
        ...
        try:
            ...
            if self.nbr_players_input:
                selected_players = \
                    self.player_controller.randomly_pick_players(
                        int(self.nbr_players_input.input.text())
                    )
                for player in selected_players:
                    self.tournament_controller.add_player(player)
                    print(
                        f"Successfully added player : {player.get_full_name()}"
                        )
            ...
            if registered_players_cell is not None:
                registered_players_cell.setText(
                    str(len(self.tournament.registered_players))
                )
            self.cell_change_listener = True
            self.toggle_buttons_visibility_check()
        except Exception as e:
        ...
        for row in range(self.selected_players_table.rowCount()):
            ...
            if isinstance(checkbox_widget, CenteredCheckBoxWidget):
                if checkbox_widget.isChecked():
                    player_item = self.player_repository.find_one_by_id(
                        self.selected_players_table.item(row, 0).text()
                    )
    ...
    def see_players(self):
        ...
        if selected_tournament and not self.players_list:
            ...
            # if (int(self.tournament.current_round)
            # > 1 and int(self.tournament.num_rounds)
            # == len(self.tournament.rounds)
            # ):
            # self.players_list.hide_delete_column()
    ...
    def _check_view_player_button_visibility(self):
        self.view_player_button.setVisible(True) if self.tournament and len(
            self.tournament.registered_players
        ) > 0 else self.view_player_button.setVisible(False)
