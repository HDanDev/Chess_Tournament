...
class PlayerCreationView(QWidget):
    def __init__(self, nav):
        ...
        self.first_name = InputField(self.layout, "First name*", "Player's first name", is_mandatory=True)
        self.last_name = InputField(self.layout, "Last name*", "Player's last name", is_mandatory=True)
    ...
    def register(self):
        if self.first_name.validate_input() and self.last_name.validate_input():
            new_player = Player()
