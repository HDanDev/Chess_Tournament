...
class InputField(QWidget):
    ...
    def create_component(self):
        if self._form_type == FormType.Normal:
            ...
            if self._is_mandatory or self._is_regex:
    ...
    def is_valid(self, value):
        if self._form_type == FormType.Numerical:
            ...
            if self._is_regex:
                ...
                if string_length in patterns and not re.match(
                    patterns[string_length], value
                ):
                    self._error_message.setText(
                        'Pattern must match the "LL0000" format'
                    )
                    return False
                elif string_length > 6:
                    self._input.setText(str(value[:6]))
