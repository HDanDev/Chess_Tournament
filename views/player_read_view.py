...
class PlayerReadView(QWidget):
    ...
    def __init__(self, nav, tournament=""):
        ...
        self.player_data = (
            ...
            if tournament != ""
                ...
                self.player_controller.get_player_data(),
    ...
    def populate_table(self):
        add_player_method = (
            self.add_player_to_table_with_score
            if self.is_sorted_by_score
            else self.add_player_to_table
        )

        for player in self.player_data:
            add_player_method(player)

        if not self.is_sorted_by_score:
    ...
    def add_player_to_table(self, player):
        ...
        date_of_birth = QTableWidgetItem(
            player.date_of_birth.toString(Qt.ISODate)
            )
        ...
        #     partial(self.delete_in_tournament, row=self.row_position)
        # ) if self.is_tournament_related else delete_btn.clicked.connect(
        #     partial(self.delete, row=self.row_position)
        # ))
        delete_btn.clicked.connect(
            self.delete_in_tournament
        ) if self.is_tournament_related else delete_btn.clicked.connect(
            self.delete
            )
    ...
    def handle_item_changed(self, item):
        ...
        try:
            ...
            edited_player.date_of_birth = QDate.fromString(
            ...
            for player in self.tournament.registered_players:
                try:
                    if player.chess_id == id:
                        ...
                        self.tournament_controller.save_changes(
                            self.tournament
                            )
                        self.table.removeRow(row)
                except Exception as e:
                    print(
                        f"An error occured while trying "
                        f"to delete the item: {e}"
                        )
