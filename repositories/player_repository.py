...
class PlayerRepository(BaseRepository):
    ...
    def add_json(self, player):
        ...
        if self.__is_id_assigned(player.chess_id, self.__get_assigned_ids()):
            raise ValueError(
                f"ID: {player.chess_id} is already in use, "
                f"the chess ID has to be unique. "
                f"Maybe the player has already been registered."
            )
        ...
        try:
            ...
            print(
                ...
                f"therefore the json file has not been updated"
            )
    ...
    def _deserialize(data):
        ...
        player.date_of_birth = (
            QDate.fromString(data["date_of_birth"], Qt.ISODate)
            )
